...
public final class CodecUtils {
	...
	public static <A> A parseNewInstanceHacky(Codec<A> codec) {
		try {
			...
			throw new IllegalArgumentException("Failed to create new instance of config in the %s mod".formatted(HLServer.MOD_NAME), e);
		}
	}
	...
	public static <T> void decode(Codec<T> codec, JsonElement o, Consumer<T> consumer) {
		try {
			...
			HLClient.LOGGER.warn("Failed to decode JsonElement:", e);
		}
	}
	...
	public static <T> T decode(String id, Codec<T> codec, JsonObject o) {
		if (o.has(id)) {
			try {
				...
				HLClient.LOGGER.warn("Failed to decode \"%s\" from JsonObject:".formatted(id), e);
			}
		}
		return null;
	}
